...
| | CV (`cv.md`) | Resume (`resume-{sre,de,mlops}.md`) |
|---|---|---|
...
**Facets:** sre = Platform / Infrastructure / SRE · de = Data Engineer · mlops = MLOps / AI Platform.
...
├── resume-sre.md          # SSOT — facet A (Platform/Infra/SRE)
...
├── resume-{sre,de,mlops}.json  # generated by `make cv-build` — input to the renderer
...
```
  portal/blog ──/cv-sync──▶ cv.md ──/cv-tailor──▶ resume-{sre,de,mlops}.md
                              │                          │
                              └──────── make cv-build ───┘
                                            │
                                            ▼
...
### 2. Targeting — `/cv-tailor <sre|de|mlops> [--jd <url>]` (assisted)
...
#   ✓ resume-sre.pdf (resume)← one-page DM
...
python3 scripts/cv_render.py docs/resume/resume-sre.json --mode resume --out a.html
...
3. To tailor a resume to a specific posting: `/cv-tailor <sre|de|mlops> --jd <url>`.
